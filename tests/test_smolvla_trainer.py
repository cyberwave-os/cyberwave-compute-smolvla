...
class TestBuildPipelineConfig:
    ...
    def test_periodic_eval_disabled(self) -> None:
        pytest.importorskip("lerobot")
        assert self._build({"max_steps": 5, "batch_size": 2}).eval_freq == 0


class TestLerobotApiCompat:
    """Guard against silent lerobot API drift.

    ``requirements.txt`` pins ``lerobot[smolvla]<0.6.0`` because 0.6.0 renamed
    ``TrainPipelineConfig.eval_freq`` to ``env_eval_freq`` (and moved
    ``WandBLogger`` — see tests/test_cw_trainer.py). Installing an unsupported
    lerobot used to surface as ``TypeError: TrainPipelineConfig.__init__() got an
    unexpected keyword argument 'eval_freq'`` only once a training job was already
    running on a GPU node; these assertions fail at test time instead.
    """

    def test_train_pipeline_config_accepts_the_kwargs_we_pass(self) -> None:
        pytest.importorskip("lerobot")
        import dataclasses
        ...
        assert expected <= fields, (
            f"installed lerobot dropped TrainPipelineConfig fields: {sorted(expected - fields)} "
            "— check the lerobot pin in requirements.txt"
        )
